...
app.post("/api/levels", async (req: any, res: any) => {
  try {
    ...
    try {
      ...
      const queryResult = db.prepare("SELECT last_insert_rowid() as id").get();
...
app.get("/api/levels", (req: any, res: any) => {
  try {
    ...
    let query = `SELECT id, name, author, created_at, sun, is_water, likes, dislikes, plays, version FROM levels`;
...
app.get("/api/levels/:id/download", (req: any, res: any) => {
  try {
    ...
    db.prepare("UPDATE levels SET plays = plays + 1 WHERE id = ?").run(levelId);
    ...
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${typedLevel.name.replace(/[^a-zA-Z0-9]/g, "_")}.izl2"`
      );
...
app.get("/api/config", (_req: any, res: any) => {
  res.json({
    ...
    turnstileSiteKey: useTurnstile ? Deno.env.get("TURNSTILE_SITE_KEY") : null,
  ...
  try {
    ...
    try {
      ...
      app.listen(Number(port), () => {
        console.log(`SSL enabled - HTTPS server running on https://localhost:${port}`);
      });
